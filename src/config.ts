...
import { GiFingerPrint, GiPocketRadio, GiBookCover, GiThermometerCold, GiLaserSparks, GiCircleSparks, GiAudioCassette } from 'react-icons/gi';

export type EvidenceTypes = 'emf' | 'temperature' | 'fingerprints' | 'writing' | 'spiritbox' | 'orb';
...
const ghosts: Ghost[] = [
	...
	},
	{
		...
		strengths: 'Almost never touch the ground. Footprint sounds from a wraith will be rare to non-existent, and they can travel directly through walls doors.',
		weaknesses: 'Have a toxic reaction to Salt. If a Wraith comes into contact with a pile of salt, it will immediately cease attacking.',
	},
	{
		name: 'Phantom',
	...
	},
	{
		...
		strengths: 'Is capable of influencing more objects at once than any other Ghosts, and is capable of shutting multiple doors at once.',
		weaknesses: 'Is almost ineffective in an empty room. ',
	},
	{
		name: 'Banshee',
		evidence: ['emf', 'fingerprints', 'temperature'],
		strengths: 'Wll focus on one player at a time until it kills them or the player leaves the building.',
		weaknesses: 'Fear the Crucifix, which boosts the Hunt-stopping range of one from 3 meters to 5 meters against it.',
	},
	{
		name: 'Jinn',
	...
	},
	{
		...
		strengths: 'Will travel at a significantly faster speed when hunting a victim. Additionally, can freely switch whoever it is targeting during a Hunt.',
